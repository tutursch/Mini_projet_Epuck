...
static uint8_t previous_width = 0, previous_mean = 0;
...
static THD_FUNCTION(ProcessImageRoad, arg) {
    ...
    while(1){
		...
		//condition to stop stop when meeting a red light
			...
			status = 0;
		//then if there is nothing, the camera sees white and we are on a pedestrian crossing
		}else{
...
uint8_t red_light(){
	...
	if ((mean_R > COLOR_MEAN_SECURITY * mean_G) & (mean_R > COLOR_MEAN_SECURITY * mean_B)){
		return 1;
	}else{
		return 0;
	}
